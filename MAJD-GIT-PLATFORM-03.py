...
async function ownerService(action){
  ...
  if(out) out.textContent='جارٍ التنفيذ...';

  try{
    const response=await fetch('/api/owner/service',{
      method:'POST',
    ...
    if(!response.ok || !data.ok){
      if(out) out.textContent='فشل التنفيذ: '+(data.error||'خطأ');
      return;
    }

    if(out){
      out.textContent='تم التنفيذ — الخدمة الآن: '+data.service;
    }
